...
const array = ['vyshali','mr bilu','msn','adarsh sir','ajmal','akhil','ambadi','aze','bilal ser','dingan','farhu','fz','nadeem','trux','tear','Aflah','Farha','mine','Filo','nexusnw','nw akhil','tear pc']
array.map( async (a) => {
let pattern = new RegExp(`\\b${a}\\b`, 'g');
if(pattern.test(message.message)){
       await message.client.sendMessage(message.jid, fs.readFileSync('./uploads/' + a + '.mp3'), MessageType.audio, { mimetype: Mimetype.mp4Audio, quoted: message.data, ptt: true})
...
const array = ['bilal ser','mr bilu','tear','tear pc','nw akhil','Filo','nexusnw']
array.map( async (a) => {
let pattern = new RegExp(`\\b${a}\\b`, 'g');
if(pattern.test(message.message)){
